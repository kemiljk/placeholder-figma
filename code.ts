...
figma.ui.onmessage = (msg) => {
  ...
  const title = ["Mr", "Mrs", "Ms", "Dr", "Rev", "Sir", "Dame", "Lord", "Lady"];
  const firstName = [
    "Aaran",
    "Aaren",
    "Aarez",
  ...
  if (msg.type === "reset-address") {
    async function resetAddress() {
      ...
      for (const node of items) {
        if (items.length >= 1 && node.type === "TEXT") {
  ...
  if (msg.type === "create-date") {
    ...
    async function date() {
      if (
        ...
        await figma.loadFontAsync(selected.fontName as FontName);
        ...
        await figma.loadFontAsync({ family: "Roboto", style: "Regular" });
        const text = figma.createText();
        text.insertCharacters(0, randomDate(new Date(2021, 0, 1), new Date()));
        nodes.push(text);
        figma.currentPage.selection = nodes;
        figma.viewport.scrollAndZoomIntoView(nodes);
      }
  ...
  if (msg.type === "create-last-name") {
    function generate_random_last_name(string_length) {
      let random_name;
      for (let i = 0; i < string_length; i++) {
        random_name = lastName[Math.floor(Math.random() * lastName.length)];
      }
      return random_name;
    }

    function generateRandomLastName() {
      return generate_random_last_name(3);
    }

    async function randomLastName() {
      if (
        selection.length === 1 &&
        figma.currentPage.selection[0].type === "TEXT"
      ) {
        ...
        const text = figma.currentPage.selection[0];
        text.insertCharacters(0, generateRandomLastName());
      } else if (
        selection.length > 1 &&
        figma.currentPage.selection[0].type === "TEXT"
      ) {
        ...
        for (const node of items) {
          if (items.length >= 1 && node.type === "TEXT") {
            node.characters = "";
            node.insertCharacters(0, generateRandomLastName());
          }
        }
      } else {
        const nodes = [];
  ...
  if (msg.type === "create-first-name") {
    function generate_random_first_name(string_length) {
      let random_name;
      for (let i = 0; i < string_length; i++) {
        random_name = firstName[Math.floor(Math.random() * firstName.length)];
      }
      return random_name;
    }

    function generateRandomFirstName() {
      return generate_random_first_name(3);
    }
    ...
    async function randomFirstName() {
      if (
        ...
        const text = figma.currentPage.selection[0];
        text.insertCharacters(0, generateRandomFirstName());
      } else if (
        selection.length > 1 &&
        figma.currentPage.selection[0].type === "TEXT"
      ) {
        ...
        for (const node of items) {
          if (items.length >= 1 && node.type === "TEXT") {
            node.characters = "";
            node.insertCharacters(0, generateRandomFirstName());
          }
        }
      } else {
        const nodes = [];
  ...
  if (msg.type === "insert-firstName") {
    ...
    async function firstNameFunc() {
      if (
        ...
        await figma.loadFontAsync(selection.fontName as FontName);
        for (const node of items) {
          if (items.length > 0 && node.type === "TEXT") {
  ...
  if (msg.type === "swap-firstName") {
    function swap_firstName(string_length) {
      let random_firstName;
      for (let i = 0; i < string_length; i++) {
        random_firstName =
          firstName[Math.floor(Math.random() * firstName.length)];
    ...
    function swapFirstName() {
      return swap_firstName(3).toLowerCase();
    }

    async function swapRandomfirstName() {
      if (
        selection.length > 0 &&
        figma.currentPage.selection[0].type === "TEXT"
      ) {
        ...
        for (const node of items) {
          if (items.length > 0 && node.type === "TEXT") {
            const textIn = node.characters;
            let textInParts = textIn.split("@");
            node.characters = swapFirstName() + "@" + textInParts[1];
          }
        }
      }
    }
